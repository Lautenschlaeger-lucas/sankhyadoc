...
assert.equal(catalog.operations.length,45);
assert.deepEqual(catalog.operations.map(o=>`${o.method} ${o.path}`).sort(),operations.sort());
assert.equal(Object.keys(catalog.schemas).length,132);
...
assert.deepEqual(catalog.requests.filter(r=>!r.operationId).map(r=>r.path),['/v1/variations']);
console.log('Verified: 45 operations, 132 schemas, 51 Postman requests, 14 guides, schema references, local links, assets and placeholder credentials.');
